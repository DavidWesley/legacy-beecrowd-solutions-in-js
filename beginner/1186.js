...
function selectedAreaBelowSecondaryDiagonaToOperateFromSquareMatrix(matrix, operation) {
	const selectedValues = []

	for (let rowIndex = 0; rowIndex < matrix.length; rowIndex++)
		for (let colIndex = matrix.length - rowIndex; colIndex < matrix.length; colIndex++)
...
function main() {
	...
	const response = selectedAreaBelowSecondaryDiagonaToOperateFromSquareMatrix(matrix, (CMD === "S" ? "S" : "M"))
