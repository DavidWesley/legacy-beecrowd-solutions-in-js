...
function main() {
	...
	for (let index = 0; index < output.length; index++) {
		const [exprA, exprB] = input[index].split("=", 2).map(e => e.replace("x", "*"))
		const distance = Math.abs(evaluateSimpleLinearExpression(exprA) - evaluateSimpleLinearExpression(exprB))
		output[index] = "Errou!".replace("rr", "r".repeat(distance))
	}
